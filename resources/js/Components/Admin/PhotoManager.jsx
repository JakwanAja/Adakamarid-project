...
export default function PhotoManager({ kos, photos }) {
    ...
    return (
        <div className="space-y-6">
            {/* Upload form */}
            <div className="p-4 rounded-xl" style={{ border: '1px dashed #E2E8F0', backgroundColor: '#F8FAFC' }}>
                <p className="text-sm font-medium mb-3" style={{ color: '#1E293B' }}>Upload Foto Baru</p>
                <form onSubmit={handleUpload} className="flex items-center gap-3 flex-wrap">
                    ...
                    <button
                        ...
                        style={{
                            backgroundColor: (processing || !data.photo) ? '#E2E8F0' : '#2563EB',
                            color: (processing || !data.photo) ? '#64748B' : '#FFFFFF',
                            cursor: (processing || !data.photo) ? 'not-allowed' : 'pointer',
                        }}
                        onMouseEnter={e => { if (!processing && data.photo) e.currentTarget.style.backgroundColor = '#1D4ED8'; }}
                        onMouseLeave={e => { if (!processing && data.photo) e.currentTarget.style.backgroundColor = '#2563EB'; }}
                    >
            ...
            {photos && photos.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    {photos.map(photo => (
                        <div
                            ...
                            style={{
                                border: photo.is_primary ? '2px solid #2563EB' : '1px solid #E2E8F0',
                                aspectRatio: '4/3',
                            }}
                        >
                            <ImageWithFallback
                ...
                <p className="text-sm italic text-center py-10" style={{ color: '#64748B' }}>
